...
With Server-Side Routing, the whole page refreshes because with the changing of a URL comes with a whole new page from the server. With Client-Side Routing, when the URL changes, the JS that has already been loaded is shown. This means no page refresh. 

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

C is for Create which is POST, R is for Read which is GET, U is for Update which is PUT, and D is for Delete which is DELETE.
